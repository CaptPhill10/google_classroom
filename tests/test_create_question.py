...
class TestOpenCoursePage:
    ...
    def invite(self, driver, test_config, text_data):
        ...
        invite.invite_people(
            teacher_login=Constants.QUESTION_TEACHER_LOGIN,
            student_login=Constants.QUESTION_STUDENT_LOGIN
        )
...
class TestClassworkPage:
    ...
    def classwork(self, driver, test_config):
        ...
        classwork.wait_for_element_clickable(
            element=classwork.question_button,
            wait_time=30
        )
        classwork.question_button.click()

        if not classwork.question_title.visible:
            classwork.question_button.click()
...
class TestQuestionPage:
    ...
    def question(self, driver, test_config, text_data):
        ...
        question.question_title.input_text(text_data.QUESTION)

        question.wait_for_element_clickable(
            element=question.due_date_button,
            wait_time=30
        )
    ...
    def test_question_title_text(self, question, text_data):
        with allure.step("Question Title text in field"):
            try:
                assert question.question_title.text == text_data.QUESTION
            except:
...
class TestQuestionCreated:
    ...
    def test_question_created(self, classwork, text_data):
        with allure.step("Question is created"):
            try:
                assert classwork.other_name.text == text_data.QUESTION
            except:
                allure.attach(classwork.driver.get_screenshot_as_png(),
